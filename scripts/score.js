// The first thing to do is to start the transport
...
game.score = (function(){
    ...
    var fluteCallback = function(time, value)
    {
      ...
      if ( shouldRest() ) {
        ...
        {
          let i = Math.floor( Math.random() * game.instruments.sampler.length );
          if(game.instruments.sampler[i].buffer.loaded)
            game.instruments.sampler[i].start( time+Tone.Time(dur).toSeconds() );
          nPhrases = 0;
        }
      }
